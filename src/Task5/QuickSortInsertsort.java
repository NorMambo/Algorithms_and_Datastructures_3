...
public class QuickSortInsertsort {

    int[] arr;
    public QuickSortInsertsort() {}
    ...
    public void _sort(int[] arr, int low, int high, int currDepth, int limit) {
        ...
        if(currDepth == limit && low != high) { // // when recursion depth is reached switch to insertsort
//            System.out.println("using Insertsort, depths : " + currDepth);
            InsertSort is = new InsertSort(arr, low, high);
            is.sort(); // sort the slice (low to high)
            return;
        }
